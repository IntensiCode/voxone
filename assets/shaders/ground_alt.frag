...
float march(vec3 ro, vec3 rd, out int max_iter) {
    ...
    {
        vec3 p = ro + d*rd;
        float h = height(p.xz, MIN_DISTANCE);
        ...
        float sl = stepLength.x;
        if (currentStep == 1) sl = stepLength.y;
...
vec3 getSample1(vec2 p) {
    float off = 0.47*iTime;

    vec3 ro  = vec3(0.5 + off, 1.0-0.25, -2.0 + off / 4);
