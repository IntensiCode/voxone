...
uniform vec2 iNoise;
uniform float iHash;
uniform float iSpaceD;
uniform float iSpace1;
uniform float iSpace2;
uniform float iSpace3;
...
vec4 hash4( vec4 n ) { return fract(sin(n)*iHash); }
float noise3( vec3 x )
{
    vec3 p = floor(x);
    vec3 f = fract(x);
    f = f*f*(3.0-2.0*f);
    float n = p.x + dot(p.yz,iNoise);
...
float noiseSpace(vec3 ray,vec3 pos,float r,mat3 mr,float zoom,vec3 subnoise)
{
    ...
    r1*=d2;
    s+=abs(noise3(vec3(r1*iSpace1+subnoise))*d);
    s+=abs(noise3(vec3(r1*iSpace2+subnoise))*d*2.0);
    s+=abs(noise3(vec3(r1*iSpace3+subnoise))*d*4.0);
    return s;
}
...
void main()
{
    ...
    vec2 sins=sin(iMouse);
    vec2 coss=cos(iMouse);
    ...
    float shadow=1.0-clamp(pow(length(sd+light*mr-rt),5.0)*200.0,0.0,0.0);
